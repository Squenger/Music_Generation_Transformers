...
def main():
    ...
    # active model
    active = MusicGeneratorPipeline(path=None, block_size=512, batch_size=16, n_embd=256, n_head=8, n_layers=8, dropout=0.1)
    active.load_model("output/music_model_best.pth")
    active_model = active.model.to(device)
    active_model.train()

    # reference model
    ref = MusicGeneratorPipeline(path=None, block_size=512, batch_size=16, n_embd=256, n_head=8, n_layers=8, dropout=0.1)
    ref.load_model("output/music_model_best.pth")
    ref_model = ref.model.to(device)
    ref_model.eval()
