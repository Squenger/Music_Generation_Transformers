...
class MusicDataset:
    def __init__(self, path, block_size, batch_size, split_ratio=0.9):
        ...
        midi_files = glob.glob(f"{path}/**/*.midi", recursive=True) + glob.glob(f"{path}/**/*.mid", recursive=True) #on récupère tous les fichier midi et mid
...
class MusicGeneratorPipeline:
    ...
    def train_model(self, epochs=50000, learning_rate=6e-4, min_lr=6e-5, warmup_iters=2000, weight_decay=0.1):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 500 == 0:
                print(f"Step {epoch:5d} | Loss: {loss.item():.4f} | LR: {lr:.4e}")
                
        print(f"Entraînement terminé - Final Loss: {loss.item():.4f}")
...
if __name__ == "__main__":
    ...
    block_size = 1024   # lonugueur du contexte musical lu d'un coup
    n_embd = 512        # dimenson des vecteurs
    ...
    max_iters = 20000        # nombre d'étapes d'entrainement
    learning_rate = 6e-4     # learning rate maximal
    min_lr = 6e-5            # learning rate minimal 
    warmup_iters = 2000      # nombre d'étapes d'echauffement
    weight_decay = 0.1       # penalisation des poids trop grands
    
    # Pipeline
    pipeline = MusicGeneratorPipeline(path="/Users/aiminemeddeb/Documents/Music_Generation_Transformers/data", block_size=block_size, batch_size=batch_size, n_embd=n_embd, n_head=n_head, n_layers=n_layer, dropout=dropout)
    ...
    pipeline.save_model("output/music_model.pth")
    
    #generation
    amorce_silence = ["Bar_None", "Position_16"]
    pipeline.generate_music(amorce_silence, max_new_tokens=1000, output_file="output/musique.mid")
